# Activations script for add the lrtaudio module to the lua path when
# building lrtaudio using the Makefile.
# Source this into interactive shell by invoking ". activates.sh" from this directory
# This is not necessary if lrtaudio is installed, e.g. via luarocks.
...
lualrtaudio_dir=$(cd "$this_dir"/..; pwd)

if [ ! -e "$lualrtaudio_dir/src/activate.sh" -o ! -e "$lualrtaudio_dir/src/main.cpp" ]; then

    echo '**** ERROR: ". activate.sh" must be invoked from "src" directory ***'

else

    echo "Setting lua paths for: $lualrtaudio_dir"

    add_lua_path="$lualrtaudio_dir/src/?.lua:$lualrtaudio_dir/src/?/init.lua"
    add_lua_cpath="$lualrtaudio_dir/src/build"
...
unset lua_cmd this_dir lualrtaudio_dir add_lua_path add_lua_cpath lua_version lua_path_vers vers default_version \
